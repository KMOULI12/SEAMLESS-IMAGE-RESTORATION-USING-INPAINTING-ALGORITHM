...
class Inpainter():
    ...
    def retrieveOriginalImage(self):
        masked_image = cv2.bitwise_and(self.inputImage, self.inputImage, mask=self.mask)
        inpainted_region = cv2.bitwise_and(self.result, self.result, mask=(255 - self.mask))
        original_image = cv2.add(masked_image, inpainted_region)
...
if __name__ == "__main__":
    ...
    originalImage = cv2.imread(
        '/content/sample_data/image1.jpg',
        1)
    ...
    # mask File Name
    inpaintMask = cv2.imread(
        '/content/sample_data/mask1.jpg',
        0)
    ...
    if i.checkValidInputs() == i.CHECK_VALID:
        ...
        original_image = i.retrieveOriginalImage()
